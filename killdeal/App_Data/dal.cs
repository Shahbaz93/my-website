...
where a.TOT_ID=@ID";
    ...
    public DataTable Boyoredrdetails()
    {
        ...
        try
        {
            ...
            cmd.Connection = con;
            cmd.Transaction = transaction;
            cmd.CommandText = @"select * from T_ECO_ORDER_TRANS a where TOT_STATUS not in('Order Place','Delivered') and TOT_ACT_FLG='Y' and TOT_CORER_ID=null order by TOT_ID desc";
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            sda.Fill(dtResult);
            Loc = dtResult;
            transaction.Commit();
    ...
    public DataTable Boyoredrdetails2(string email)
    {
        ...
        try
        {
            ...
            cmd.Connection = con;
            cmd.Transaction = transaction;
            cmd.CommandText = @"select * from T_ECO_ORDER_TRANS a where TOT_ID=@email and TOT_STATUS not in('Order Place','Delivered') and TOT_ACT_FLG='Y' and TOT_CORER_ID=null order by TOT_ID desc";
            cmd.Parameters.AddWithValue("email", Convert.ToInt32( email));
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            sda.Fill(dtResult);
            Loc = dtResult;
